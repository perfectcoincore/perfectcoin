 ...
 * between perfectcoind, and perfectcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    FlushWallets();
#endif
    GeneratePerfectCoins(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PERFECTCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/perfectcoincore/perfectcoin>";
    const std::string URL_WEBSITE = "<http://perfectcoininfo.com>";
...
/** Sanity checks
 *  Ensure that PerfectCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PerfectCoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single PerfectCoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", PERFECTCOIN_CONF_FILENAME)).string());
    ...
    GeneratePerfectCoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
