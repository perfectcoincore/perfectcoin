...
#include "perfectcoinamountfield.h"
...
public:
    explicit AmountSpinBox(QWidget *parent):
        ...
        currentUnit(PerfectCoinUnits::PTC),
    ...
    void fixup(QString &input) const
    {
        ...
        if(valid)
        {
            input = PerfectCoinUnits::format(currentUnit, val, false, PerfectCoinUnits::separatorAlways);
    ...
    {
        lineEdit()->setText(PerfectCoinUnits::format(currentUnit, value, false, PerfectCoinUnits::separatorAlways));
        Q_EMIT valueChanged();
    }
    ...
    void stepBy(int steps)
    {
        ...
        val = qMin(qMax(val, CAmount(0)), PerfectCoinUnits::maxMoney());
        setValue(val);
    }
    ...
    QSize minimumSizeHint() const
    {
        if(cachedMinimumSizeHint.isEmpty())
        {
            ...
            int w = fm.width(PerfectCoinUnits::format(PerfectCoinUnits::PTC, PerfectCoinUnits::maxMoney(), false, PerfectCoinUnits::separatorAlways));
...
private:
    ...
    {
        CAmount val = 0;
        bool valid = PerfectCoinUnits::parse(currentUnit, text, &val);
        if(valid)
        {
            if(val < 0 || val > PerfectCoinUnits::maxMoney())
                valid = false;
        }
        if(valid_out)
            *valid_out = valid;
...
protected:
    ...
    StepEnabled stepEnabled() const
    {
        ...
        if(valid)
        {
            if(val > 0)
                rv |= StepDownEnabled;
            if(val < PerfectCoinUnits::maxMoney())
                rv |= StepUpEnabled;
        }
        return rv;
    }
...
#include "perfectcoinamountfield.moc"
    ...
    amount(0)
{
    ...
    unit->setModel(new PerfectCoinUnits(this));
...
void PerfectCoinAmountField::unitChanged(int idx)
{
    ...
    int newUnit = unit->itemData(idx, PerfectCoinUnits::UnitRole).toInt();
