...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 PTC) in its wallet.
...
with the pigeond(s) being tested (using python's asyncore package); the other
implements the test logic.

- `NodeConn` is the class used to connect to a pigeond.  If you implement
...
option to change the pigeond binary used by the test.  If using 2 or more nodes,
then `--refbinary` can be optionally used to change the pigeond that will be used
on nodes 2 and up.
...
Each `TestInstance` consists of:
  - A list of `[object, outcome, hash]` entries
    * `object` is a `CBlock`, `CTransaction`, or
    ...
    complete headers-chains when requested from the pigeond, to allow writing
    tests where blocks can be delivered out of order but still processed by
    headers-first pigeond's.
    * `outcome` is `True`, `False`, or `None`.  If `True`
    or `False`, the tip is compared with the expected tip -- either the
    block passed in, or the hash specified as the optional 3rd entry.  If
    `None` is specified, then the test will compare all the pigeond's
  ...
  - `sync_every_transaction`: `True/False`.  Analogous to
    ...
    then the contents of the entire mempool are compared across all pigeond
...
Basic code to support P2P connectivity to a pigeond.
