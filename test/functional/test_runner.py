...
def main():
    ...
    tmpdir = "%s/perfectcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_pigeond = config["components"].getboolean("ENABLE_PERFECTCOIND")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    #Set env vars
    if "PERFECTCOIND" not in os.environ:
        os.environ["PERFECTCOIND"] = build_dir + '/src/pigeond' + exeext
        os.environ["PERFECTCOINCLI"] = build_dir + '/src/pigeon-cli' + exeext
